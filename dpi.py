    ...
    for n in circuit.multigraph.nodes:
        ...
        for ne in circuit.multigraph.neighbors(n):
            for k in circuit.multigraph.get_edge_data(n, ne):
                if not isinstance(circuit.multigraph.edges[n, ne, k]['component'],
                                  cir.VoltageDependentCurrentSource) and not isinstance(
                        circuit.multigraph.edges[n, ne, k]['component'], cir.VoltageSource) and not isinstance(
                        circuit.multigraph.edges[n, ne, k]['component'], cir.CurrentSource):
                    impedance += " + " + (
                        "(s*" + circuit.multigraph.edges[n, ne, k]['component'].name + ")" if isinstance(
                            circuit.multigraph.edges[n, ne, k]['component'], cir.Capacitor) else "1/" +
                                                                                                 circuit.multigraph.edges[
                                                                                                     n, ne, k][
                                                                                                     'component'].name)
                if ne != "0":
                    # print("not ground!")
                    ...
                    if not isinstance(circuit.multigraph.edges[n, ne, k]['component'],
                        ...
                        if sfg.graph.has_edge(cur_source, cur_target):
                            sfg.graph.edges[cur_source, cur_target]['weight'] += " + " + (
                                "(s*" + circuit.multigraph.edges[n, ne, k]['component'].name + ")" if isinstance(
                                    circuit.multigraph.edges[n, ne, k]['component'], cir.Capacitor) else "1/" +
                                                                                                         circuit.multigraph.edges[
                                                                                                             n, ne, k][
                            ...
                            sfg.graph.add_edge(cur_source, cur_target, weight="+" + (
                                "(s*" + circuit.multigraph.edges[n, ne, k]['component'].name + ")" if isinstance(
                                    circuit.multigraph.edges[n, ne, k]['component'], cir.Capacitor) else "1/" +
                                                                                                         circuit.multigraph.edges[
                                                                                                             n, ne, k][
                        ...
                        cur_source_1 = "V" + pos_input_node.lower() if not pos_input_node.startswith(
                            "V") else pos_input_node
                        if sfg.graph.has_edge(cur_source_1, cur_target):
                            sfg.graph.edges[cur_source_1, cur_target]['weight'] += (" - " if n ==
                                                                                             circuit.multigraph.edges[
                                                                                                 n, ne, k][
                                                                                                 'component'].pos_node else " + ") + "gm_" + str(
                                circuit.multigraph.edges[n, ne, k]['component'].name[-1])
                        else:
                            sfg.graph.add_edge(cur_source_1, cur_target, weight=(" - " if n == circuit.multigraph.edges[
                                n, ne, k]['component'].pos_node else " + ") + "gm_" + str(
                                circuit.multigraph.edges[n, ne, k]['component'].name[-1]))

                        cur_source_2 = "V" + neg_input_node.lower() if not neg_input_node.startswith(
                            "V") else neg_input_node

                        if sfg.graph.has_edge(cur_source_2, cur_target):
                            sfg.graph.edges[cur_source_2, cur_target]['weight'] += (" + " if n ==
                                                                                             circuit.multigraph.edges[
                                                                                                 n, ne, k][
                                                                                                 'component'].pos_node else " - ") + "gm_" + str(
                                circuit.multigraph.edges[n, ne, k]['component'].name[-1])
                        else:
                            sfg.graph.add_edge(cur_source_2, cur_target, weight=(" + " if n == circuit.multigraph.edges[
                                n, ne, k]['component'].pos_node else " - ") + "gm_" + str(
                                circuit.multigraph.edges[n, ne, k]['component'].name[-1]))
        if impedance != "1/(":
            impedance += ")"
...
class SFGraph(object):
    ...
    def generate_adjacent(self):
        ...
        for edge in self.edge_list:
            if self.nodes_name_map[edge.target].node_number not in self.adjacency[
                self.nodes_name_map[edge.source].node_number]:
                self.adjacency[self.nodes_name_map[edge.source].node_number].append(
                    self.nodes_name_map[edge.target].node_number)
...
class Node:
    def __init__(self, **kwargs):
        ...
        self.adj_nodes = []
        self.node_number = 0
        self.voltage = 0 if kwargs["is_ground"] is True else "V" + kwargs["node_name"].lower() if not kwargs[
            "node_name"].startswith("V") else kwargs["node_name"]
        self.short_circuit_I = "0"
        self.DPImpedence = "0"
        self.node_name = "V" + kwargs["node_name"].lower() if not kwargs["node_name"].startswith("V") and not kwargs[
            "node_name"].startswith("I") else kwargs["node_name"]
        if self.voltage == 0:
            return
    ...
    def __repr__(self):
        if self.voltage == 0:
            return "ground"
        return " NodeInfor: node_id " + str(
            self.node_number) + " node_name " + self.node_name  # +" voltage: " + self.voltage + "\n short circuit current: "+ self.short_circuit_I + "\n  driving point impedence:" + self.DPImpedence
    ...
    def DPI_analysis(self):
        ...
        for component, node in self.components.items():
            if not isinstance(component, VoltageSource) and not isinstance(component, CurrentSource):
                impedence += ((("(1/s" + component.name + ")") if isinstance(component,
                                                                             Capacitor) else component.name) + "//")
...
def construct_graph(circuit: cir.Circuit):
    ...
    for edge in circuit.multigraph.edges(keys=True, data='component'):
        ...
        if isinstance(component_object, cir.Resistor):
            ...
            circuit_components[component_name] = Capacitor(name=component_name,
